...
export function ExerciseDetail({ exercise, favorite, onClose, onFavorite, onAdd, onOpen }: { exercise: Exercise; favorite: boolean; onClose: () => void; onFavorite: () => void; onAdd: () => void; onOpen: () => void }) {
  ...
  const instructions = turkishExerciseInstructions(exercise);
  return <div className="exercise-detail-overlay" role="dialog" aria-modal="true" aria-labelledby="exercise-detail-title" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><article className="exercise-detail" ref={dialogRef}><button type="button" className="detail-close" aria-label="Egzersiz detayını kapat" onClick={onClose} autoFocus>×</button><ExerciseAnimation images={exercise.images} name={exercise.name} intervalMs={800} /><div className="detail-copy"><div className="eyebrow">EGZERSİZ VERİ TABANI</div><h2 id="exercise-detail-title">{exercise.name}</h2><div className="detail-tags"><span>{translateExerciseList(exercise.primaryMuscles) || "Genel"}</span><span>{translateExerciseLabel(exercise.equipment)}</span><span>{translateExerciseLabel(exercise.level)}</span></div><div className="exercise-metadata"><div><span>YARDIMCI KASLAR</span><strong>{translateExerciseList(exercise.secondaryMuscles) || "—"}</strong></div><div><span>KATEGORİ</span><strong>{translateExerciseLabel(exercise.category)}</strong></div><div><span>MEKANİK</span><strong>{translateExerciseLabel(exercise.mechanic)}</strong></div><div><span>KUVVET TÜRÜ</span><strong>{translateExerciseLabel(exercise.force)}</strong></div></div><section className="detail-instructions"><h3>Adım adım nasıl yapılır?</h3><ol>{instructions.map((instruction, index) => <li key={`${exercise.id}-${index}`}>{instruction}</li>)}</ol></section><div className="detail-actions"><button type="button" className="detail-open" onClick={onOpen}>▶ Hareketi aç</button><button type="button" className="detail-add" onClick={onAdd}>＋ Antrenmanıma ekle</button><button type="button" className={favorite ? "detail-favorite active" : "detail-favorite"} aria-pressed={favorite} onClick={onFavorite}>{favorite ? "♥ Favorilerimde" : "♡ Favorilere ekle"}</button></div></div></article></div>;
}
