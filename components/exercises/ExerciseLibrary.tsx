...
export function ExerciseLibrary({ onOpenWorkout, onAddWorkout }: { onOpenWorkout: (exercise: Exercise) => void; onAddWorkout: (exercise: Exercise) => boolean }) {
  ...
  const addWorkout = (exercise: Exercise) => {
    const added = onAddWorkout(exercise);
    setAddedName(added ? exercise.name : "");
    setRejectedName(added ? "" : exercise.name);
  ...
  return <div className="subview database-library"><div className="eyebrow">{t.exerciseLibrary.eyebrow}</div><h1>{t.exerciseLibrary.title1}<br /><em>{t.exerciseLibrary.title2}</em></h1><p className="lead">{t.exerciseLibrary.lead(getAllExercises().length)}</p><ExerciseFilters filters={filters} options={options} onChange={updateFilters} onClear={() => updateFilters({})} /><div className="library-result-row"><strong>{t.exerciseLibrary.resultCount(exercises.length)}</strong><span>{isPending ? t.exerciseLibrary.filtering : t.exerciseLibrary.localImages}</span></div>{exercises.length ? <div className="database-exercise-grid">{exercises.slice(0, visibleCount).map((exercise) => <ExerciseCard exercise={exercise} favorite={favorites.includes(exercise.id)} onDetail={() => setSelected(exercise)} onFavorite={() => toggleFavorite(exercise.id)} key={exercise.id} />)}</div> : <div className="library-empty"><strong>{t.exerciseLibrary.noExercisesFound}</strong><p>{t.exerciseLibrary.tryDifferentFilters}</p><button type="button" onClick={() => updateFilters({})}>{t.exerciseLibrary.clearFilters}</button></div>}{visibleCount < exercises.length && <button type="button" className="load-exercises" onClick={() => setVisibleCount((count) => count + 24)}>{t.exerciseLibrary.loadMore}</button>}{selected && <ExerciseDetail exercise={selected} favorite={favorites.includes(selected.id)} onClose={() => setSelected(null)} onFavorite={() => toggleFavorite(selected.id)} onAdd={() => addWorkout(selected)} onOpen={() => onOpenWorkout(selected)} />}{addedName && <div className="exercise-added" role="status">{t.exerciseLibrary.addedToWorkout(addedName)}</div>}{rejectedName && <div className="exercise-added" role="alert">{t.exerciseLibrary.notAddedToWorkout(rejectedName)}</div>}</div>;
}
