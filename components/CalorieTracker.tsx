...
type FoodEntry = { id: string; name: string; meal: Meal; calories: number; protein: number; carbs: number; fat: number; fiber?: number; grams?: number; portionCount?: number; amountBasis?: "grams" | "serving"; micros?: FoodMicronutrients; time: string; consumedAt: string; source: "Barkod" | "Fotoğraf" | "Manuel"; isEstimated?: boolean };
type PhotoDetectedItem = {
  name: string;
  ...
  nutrition: Omit<FoodNutrition, "micros"> | null;
  selected: boolean;
};
...
async function optimizedFoodPhoto(file: File) {
  const original = await new Promise<string>((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => typeof reader.result === "string" ? resolve(reader.result) : reject(new Error("Fotoğraf okunamadı"));
...
export function CalorieTracker({ userId, bmr = 1600, tdee = 2100, weightKg = 70, activityFactor = 1.375, workoutDays = 3, profileGoal = "" }: CalorieTrackerProps) {
  ...
  const [amountBasis, setAmountBasis] = useState<"grams" | "serving">("grams");
  ...
  const [photoDetectedItems, setPhotoDetectedItems] = useState<PhotoDetectedItem[]>([]);
  ...
  }, [activeMethod, foodName, locale, selectedProduct?.name, t]);
  ...
  async function addEntry(
    entry: Omit<FoodEntry, "id" | "time" | "consumedAt">,
    options: { productBarcode?: string; foodId?: string | null; confidence?: number | null } = {},
  ) {
    const now = new Date();
    ...
    if (userId) {
      ...
      const selectedFoodId = selectedProduct && /^[0-9a-f-]{36}$/i.test(selectedProduct.id) ? selectedProduct.id : null;
      const foodId = options.foodId && /^[0-9a-f-]{36}$/i.test(options.foodId) ? options.foodId : selectedFoodId;
      try {
        const response = await authorizedFetch("/api/nutrition/logs", {
          ...
          body: JSON.stringify({
            ...
            portionGrams: record.amountBasis === "serving" ? null : record.grams || 100,
            ...
            inputMethod,
            confidence: options.confidence ?? (aiEstimate ? (aiEstimate.confidence === "high" ? 0.9 : aiEstimate.confidence === "medium" ? 0.7 : 0.45) : null),
            isEstimated: Boolean(record.isEstimated || aiEstimate),
            metadata: {
              barcode: options.productBarcode || null,
              micros: record.micros || {},
              searchPersonalized: Boolean(selectedProduct?.personalized),
              searchScore: selectedProduct?.matchScore || null,
              amountBasis: record.amountBasis || "grams",
              portions: record.portionCount || null,
            },
          }),
        });
        ...
        if (foodId) {
          void authorizedFetch("/api/nutrition/selections", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              foodId,
        ...
        // Ağ geçici olarak kesilse bile yukarıdaki iyimser kayıt cihazda
        // ve localStorage'da kalır; kullanıcı ekleme işlemini kaybetmez.
        setMessage(t.calorieTracker.syncedLocallyOnly);
      }
    }
    setFoodName(""); setGrams("100"); setAmountBasis("grams"); setNutrition(emptyFoodNutrition()); setSelectedProduct(null); setSearchResults([]); setBarcode(""); setPhotoPreview(null); setPhotoDetectedItems([]); setAiEstimate(null);
  }
  ...
  async function submitManual() {
    ...
    const validation = validateManualNutrition({ portionGrams: amountBasis === "serving" ? Math.min(5_000, amount * 100) : amount, calories: nutrition.calories, protein: nutrition.protein, carbohydrates: nutrition.carbs, fat: nutrition.fat, fiber: nutrition.fiber });
    ...
    try {
      await addEntry({ name: foodName.trim(), meal, calories: nutrition.calories, protein: nutrition.protein, carbs: nutrition.carbs, fat: nutrition.fat, fiber: nutrition.fiber, grams: amountBasis === "grams" ? amount : undefined, portionCount: amountBasis === "serving" ? amount : undefined, amountBasis, micros: nutrition.micros, source: activeMethod === "photo" ? "Fotoğraf" : selectedProduct?.barcode ? "Barkod" : "Manuel", isEstimated: Boolean(aiEstimate || selectedProduct?.needsReview) }, { productBarcode: selectedProduct?.barcode });
  ...
  async function findBarcode() {
    ...
    const result = await response.json().catch(() => ({})) as { id?: string; error?: string; name?: string; brand?: string; barcode?: string; servingGrams?: number; source?: FoodSearchResult["source"]; verified?: boolean; dataQuality?: FoodSearchResult["dataQuality"]; nutritionPer100g?: FoodNutrition };
    ...
    setAmountBasis("grams");
    setNutrition(scaleFoodNutrition(result.nutritionPer100g, serving));
    setSelectedProduct({ id: result.id || `barcode-${barcode}`, name: result.name, brand: result.brand, barcode: result.barcode || barcode, servingGrams: serving, nutritionPer100g: result.nutritionPer100g, source: result.source || "Open Food Facts", verified: result.verified, dataQuality: result.dataQuality });
    setAiEstimate(null);
  ...
  async function handlePhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    event.target.value = "";
    const photoDataUrl = await optimizedFoodPhoto(file).catch(() => "");
  ...
  async function analyzeFoodPhoto(photoDataUrl: string, previewUrl = photoDataUrl) {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({})) as {
        ...
        detectedItems?: Array<Omit<PhotoDetectedItem, "selected">>;
      };
      ...
      setSelectedProduct(null);
      const detectedItems = (result.detectedItems || []).map((item) => ({ ...item, selected: Boolean(item.nutrition) }));
      setPhotoDetectedItems(detectedItems);
      ...
      const manualNotice = result.needsManualNutrition ? ` ${t.calorieTracker.photoManualNutrition}` : "";
      ...
      setMessage(t.calorieTracker.photoAnalysisFailed);
    } finally {
      setAnalyzingPhoto(false);
    }
  }

  function syncPhotoSummary(items: PhotoDetectedItem[]) {
    const selected = items.filter((item) => item.selected && item.nutrition);
    ...
    setFoodName(selected.map((item) => item.name).join(", "));
    setGrams(String(Math.round(totals.grams)));
    setNutrition({
  ...
  function togglePhotoItem(index: number, selected: boolean) {
    const next = photoDetectedItems.map((item, itemIndex) => itemIndex === index ? { ...item, selected } : item);
    setPhotoDetectedItems(next);
    syncPhotoSummary(next);
  }

  function choosePhotoCandidate(index: number, candidate: PhotoDetectedItem["candidates"][number]) {
    const next = photoDetectedItems.map((item, itemIndex) => {
      if (itemIndex !== index) return item;
      const candidateNutrition = candidate.nutritionPer100g
        ? scaleFoodNutrition(candidate.nutritionPer100g, item.grams)
        : null;
      return {
        ...item,
        ...
        variantSummary: candidate.variantSummary,
        needsReview: candidate.needsReview,
        nutrition: candidateNutrition ? {
          calories: candidateNutrition.calories,
          protein: candidateNutrition.protein,
          carbs: candidateNutrition.carbs,
          fat: candidateNutrition.fat,
          fiber: candidateNutrition.fiber,
        } : null,
        selected: Boolean(candidateNutrition),
      };
    });
    setPhotoDetectedItems(next);
    syncPhotoSummary(next);
  ...
  function updatePhotoItemGrams(index: number, value: string) {
    const nextGrams = Number(value.replace(",", "."));
    if (!Number.isFinite(nextGrams) || nextGrams <= 0 || nextGrams > 5_000) return;
    const next = photoDetectedItems.map((item, itemIndex) => {
      if (itemIndex !== index || !item.nutrition || item.grams <= 0) return item;
      ...
      return {
        ...
        nutrition: {
          ...
          protein: Math.round(item.nutrition.protein * factor * 10) / 10,
          carbs: Math.round(item.nutrition.carbs * factor * 10) / 10,
          fat: Math.round(item.nutrition.fat * factor * 10) / 10,
          fiber: Math.round(item.nutrition.fiber * factor * 10) / 10,
        },
      };
    });
    setPhotoDetectedItems(next);
  ...
  async function submitPhotoMeal() {
    const selectedItems = photoDetectedItems.filter((item) => item.selected && item.nutrition);
    if (isSubmitting || selectedItems.length === 0) {
      if (selectedItems.length === 0) setMessage(t.calorieTracker.photoNoSelected);
      return;
    }
    setIsSubmitting(true);
    try {
      for (const item of selectedItems) {
        ...
        await addEntry({
          ...
          foodId: item.catalogId && /^[0-9a-f-]{36}$/i.test(item.catalogId) ? item.catalogId : null,
          confidence: item.confidence,
        });
      }
      setMessage(t.calorieTracker.photoEntriesAdded(selectedItems.length, translateMeal(t, meal)));
    } finally {
      setIsSubmitting(false);
    }
  }
  ...
  function updateGrams(value: string) {
    ...
    if (selectedProduct?.nutritionPerServing && amountBasis === "serving") {
      setNutrition(scaleFoodNutrition(selectedProduct.nutritionPerServing, next * 100));
      return;
    }
    if (Number.isFinite(previous) && previous > 0) setNutrition((current) => scaleFoodNutrition(current, (next / previous) * 100));
  }
  ...
  function selectProduct(product: FoodSearchResult) {
    if (!product.nutritionPer100g) {
      if (product.nutritionPerServing) {
        ...
        setGrams("1");
        setAmountBasis("serving");
        setNutrition(product.nutritionPerServing);
        setSelectedProduct(product);
        setSearchResults([]);
        setSearchState("idle");
        setSearchNotice("");
        setMessage(locale === "en"
          ? "Source serving values loaded. Check the portion count; this record is low confidence."
          : "Kaynak porsiyon değerleri yüklendi. Porsiyon adedini kontrol edin; kayıt düşük güvenlidir.");
        return;
      }
      setFoodName(product.name);
      setSelectedProduct(null);
      setMessage(product.nutritionPerServing
        ? (locale === "en"
          ? "A source serving is available, but its gram weight is not verified. Enter the gram weight and values manually."
          : "Kaynak porsiyon makroları var; ancak gram ağırlığı doğrulanmadı. Gramı ve değerleri elle girebilirsiniz.")
        : (locale === "en"
    ...
    setSearchState("idle");
    setSearchNotice("");
    setMessage(t.calorieTracker.productLoadedMessage);
  }
  ...
  return <div className="calorie-tracker subview">
    ...
    <section className="food-entry-panel">
      ...
      <div className="entry-workspace">
        ...
        {activeMethod === "photo" && <div className="method-content photo-food-content"><button type="button" className="food-photo-drop" onClick={() => void chooseFoodPhoto()} disabled={analyzingPhoto}>{photoPreview ? <Image src={photoPreview} alt={t.calorieTracker.photoAlt} width={640} height={480} unoptimized /> : <><ImagePlus size={26} /><strong>{t.calorieTracker.photoDropPrompt1}</strong><span>{t.calorieTracker.photoDropPrompt2}</span></>}</button><input ref={cameraInput} className="sr-only" type="file" accept="image/*" capture="environment" onChange={handlePhoto} />{photoPreview && <button type="button" className="outline-btn" disabled={analyzingPhoto} onClick={() => void chooseFoodPhoto()}>{t.calorieTracker.choosePhotoAgain}</button>}</div>}
        {activeMethod === "photo" && photoDetectedItems.length > 0 && <div className="photo-analysis-card" aria-live="polite">
          <div className="photo-analysis-head"><div><span>{t.calorieTracker.photoDetectedTitle}</span><strong>{t.calorieTracker.photoDetectedCount(photoDetectedItems.length)}</strong></div><small>{t.calorieTracker.photoSelectedMeal(translateMeal(t, meal))}</small></div>
          <div className="photo-detected-list">{photoDetectedItems.map((item, index) => <div className={`photo-detected-item ${item.selected ? "selected" : ""} ${item.matchKind}`} key={`${item.name}-${index}`}>
            <label className="photo-item-toggle"><input type="checkbox" checked={item.selected} disabled={!item.nutrition} onChange={(event) => togglePhotoItem(index, event.target.checked)} /><span><strong>{item.catalogName || item.name}</strong><small>{item.matchKind === "approximate" ? t.calorieTracker.photoItemApproximate(item.catalogName || item.name) : item.matchKind === "unmatched" ? t.calorieTracker.photoItemUnavailable : item.needsReview && !item.nutrition ? (locale === "en" ? "Nutrition review required" : "Besin hesabı inceleme bekliyor") : item.preparation || ""}</small>{item.variantSummary && <small>{item.variantSummary}</small>}</span></label>
            {item.candidates.length > 1 && <label><span>{locale === "en" ? "Catalog match" : "Katalog eşleşmesi"}</span><select value={item.catalogId || ""} onChange={(event) => { const candidate = item.candidates.find((option) => option.id === event.target.value); if (candidate) choosePhotoCandidate(index, candidate); }}>{item.candidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}{candidate.nutritionPer100g ? "" : ` · ${locale === "en" ? "review" : "inceleme"}`}</option>)}</select></label>}
            {item.nutrition && <label className="photo-item-grams"><span>{t.calorieTracker.photoItemGrams}</span><input inputMode="decimal" value={formatAmount(item.grams, dateLocale)} onChange={(event) => updatePhotoItemGrams(index, event.target.value)} /></label>}
            <b>{item.nutrition ? `${formatAmount(item.nutrition.calories, dateLocale)} kcal` : "—"}</b>
          </div>)}</div>
          <button type="button" className="primary-btn photo-add-meal" disabled={isSubmitting || !photoDetectedItems.some((item) => item.selected && item.nutrition)} onClick={() => void submitPhotoMeal()}><Plus size={17} /> {isSubmitting ? t.calorieTracker.photoAdding : t.calorieTracker.photoAddToMeal(translateMeal(t, meal), photoDetectedItems.filter((item) => item.selected && item.nutrition).length)}</button>
        </div>}
        {(activeMethod === "manual" || (photoPreview && photoDetectedItems.length === 0 && !analyzingPhoto)) && <>
          <div className="manual-fields">
            <label className="food-name">{t.calorieTracker.foodNameLabel}<input value={foodName} onChange={(event) => { setFoodName(event.target.value); setAmountBasis("grams"); setSelectedProduct(null); setAiEstimate(null); }} placeholder={t.calorieTracker.foodNamePlaceholder} autoComplete="off" /></label>
            <label>{amountBasis === "serving" ? (locale === "en" ? "Portions" : "Porsiyon adedi") : t.calorieTracker.portionLabel}<input inputMode="decimal" value={grams} onChange={(event) => updateGrams(event.target.value)} placeholder={amountBasis === "serving" ? "1" : "100"} /></label>
          ...
          {activeMethod === "manual" && foodName.trim().length >= 2 && <div className={`food-search-panel ${searchState}`} aria-live="polite">
            ...
            {searchState === "ready" && <div className="food-search-results">{searchResults.map((product) => <button type="button" key={product.id} onClick={() => selectProduct(product)} title={product.variantSummary}><span><strong>{product.name}</strong><small>{[product.brand, product.category, product.canonicalFamily, product.personalized ? (locale === "en" ? "For you" : "Sana özel") : "", product.needsReview ? (locale === "en" ? "Review required" : "İnceleme gerekli") : ""].filter(Boolean).join(" · ")}</small>{product.variantSummary && <small>{product.variantSummary}</small>}</span>{product.nutritionPer100g ? <b>{product.nutritionPer100g.calories} kcal<small>/100 g</small></b> : product.nutritionPerServing ? <b>{product.nutritionPerServing.calories} kcal<small>/{product.portionLabel || "porsiyon"}</small></b> : <b className="nutrition-pending">{locale === "en" ? "Nutrition unavailable" : "Besin değeri bulunamadı"}</b>}</button>)}</div>}
