...
export function WorkoutCalendar({ active, userId, onStartWorkout }: WorkoutCalendarProps) {
  ...
  useEffect(() => {
    ...
    const permissionTimer = window.setTimeout(() => setNotificationPermission(typeof Notification === "undefined" ? "unsupported" : Notification.permission), 0);
  ...
  useEffect(() => {
    ...
    async function loadCalendar() {
      ...
      if (preferenceRow) setPreferences({ workoutDays: Array.isArray(preferenceRow.workout_days) ? preferenceRow.workout_days.map(Number) : defaultPreferences.workoutDays, preferredTime: String(preferenceRow.preferred_time || defaultPreferences.preferredTime).slice(0, 5), reminderMinutesBefore: Number(preferenceRow.reminder_minutes_before || 30), browserNotifications: Boolean(preferenceRow.browser_notifications) });
      setEntries((scheduleRows || []).map((row) => scheduleFromRow(row as Record<string, unknown>)));
    }
    void loadCalendar();
  ...
  useEffect(() => {
    if (!upcoming || !reminderVisible || !preferences.browserNotifications || typeof Notification === "undefined" || Notification.permission !== "granted") return;
  ...
  async function savePreferences(nextPreferences = preferences) {
    ...
    setEntries([...retainedEntries, ...(inserted || []).map((row) => scheduleFromRow(row as Record<string, unknown>))].sort((a, b) => a.scheduledDate.localeCompare(b.scheduledDate)));
    setMessage("Takvim ve hatırlatma tercihlerin kaydedildi.");
  }

  async function requestNotificationPermission() {
    if (typeof Notification === "undefined") { setError("Bu tarayıcı bildirimleri desteklemiyor. Uygulama içi hatırlatmalar devam edecek."); return; }
    const permission = await Notification.requestPermission();
    setNotificationPermission(permission);
    const next = { ...preferences, browserNotifications: permission === "granted" };
    setPreferences(next);
    await savePreferences(next);
    if (permission === "granted") setMessage("Tarayıcı bildirimleri açıldı.");
    else setError("Bildirim izni verilmedi. İstersen tarayıcı ayarlarından daha sonra açabilirsin.");
  }
  ...
  return <div className="calendar-view subview">{reminderBanner}<div className="eyebrow">ANTRENMAN TAKVİMİ</div><div className="calendar-title-row"><div><h1>Haftanı <em>ritmine göre planla.</em></h1><p className="lead">Antrenman günlerini seç, saatini ayarla ve kaçırdığın günleri kolayca yeniden planla.</p></div><span className="timezone-badge">Europe/Istanbul · UTC+3</span></div>
    ...
    <section className="notification-card"><div><span className="notification-icon">◉</span><div><strong>Tarayıcı bildirimi</strong><p>Uygulama açıkken yaklaşan antrenmanını bildirir. Kapalı uygulamada arka plan bildirimi bu ilk aşamaya dahil değildir.</p></div></div><div><span className={`permission-status ${notificationPermission}`}>{notificationPermission === "granted" ? "İzin verildi" : notificationPermission === "denied" ? "Engellendi" : notificationPermission === "unsupported" ? "Desteklenmiyor" : "İzin bekliyor"}</span>{notificationPermission !== "granted" && notificationPermission !== "unsupported" && <button type="button" onClick={() => void requestNotificationPermission()}>Bildirim izni ver</button>}</div></section>
    ...
    <aside className="background-reminder-note"><strong>Arka plan bildirimi hakkında</strong><p>Uygulama kapalıyken güvenilir bildirim için servis worker, web push aboneliği ve sunucu tarafında zamanlanmış gönderim gerekir. Bu sürüm uygulama içi uyarıyı ve açık tarayıcı bildirimini eksiksiz uygular.</p></aside>
